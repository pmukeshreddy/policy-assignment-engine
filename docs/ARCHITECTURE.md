...
The NYC importer fetches exactly 50,000 validated records from dataset `k397-673e` and persists normalized facts plus query/fetch/checksum provenance. The offline regression runner invokes the same observed-data baseline builder used by product initialization, establishes the coherent 300-rule universe through the normal FULL job, and compares it to an independent exhaustive interpreter before mutation testing begins.

For the current population, the shared builder produces 146 policies: 44 observed department workflow policies, 7 workplace policies (one shared plus all 6 observed locations), 4 employment/compensation policies, 37 deterministic title-family policies covering all 304 observed titles, 4 tenure-tier policies, and 50 cross-functional policies. Each of those six domains contributes exactly 50 rules. Policy count is a consequence of observed population semantics; the 300-rule scale is fixed.

At least 100,000 deterministic API mutations then exercise employee facts, tenure/date movement, groups, rules and versions, policy versions, manual assignment/exclusion/revocation, and duplicate job delivery. Ordinary localized employee/group mutations accumulate into verification batches of up to 1,000 before their affected scopes are compared with the oracle. Global, temporal, rule, and policy configuration mutations remain singleton checkpoints and compare all 50,000 employees immediately. The 1,000-mutation verification bound does not change the worker's separate 500-scope reconciliation transaction size.

The production path records reconciliation-job processing time, affected employees/scopes, and evaluation counts. Localized latency starts when the corresponding job begins processing and ends after assignment materialization and successful job completion; evaluator batch residence and oracle verification are excluded. The dedicated isolated benchmark additionally measures API commit-to-visible latency. Human-readable and machine-readable summaries are generated under `eval-results/`; large per-batch replay ledgers and failure artifacts remain local. The five headline metrics are assignment correctness, business-transition conformance, localized end-to-end convergence latency, population fan-out completion, and rule-evaluation work avoided. Correctness invariants are gates under assignment correctness; outcome counts are diagnostics under business conformance. Operational batch, worker, and total-runtime details are retained only in the JSON debug section. No NYC API request occurs during regression execution.
...
The idempotent `seed:product` command creates **NYC Open Data Policy Workspace** by copying the immutable normalized facts in `employee_import_records` into a separate tenant with product-owned employee/version and provenance rows. It invokes the shared `createCertifiedBaseline()` implementation—the same code called by evaluation setup—to create six categories, a data-derived policy count, exactly 300 rules, and observed groups, then drains one company-scoped full job through `ReconciliationWorker`. It never writes materialized assignments directly and never calls the NYC API. `GET /companies` excludes `evaluation_tenants` and orders the isolated product tenant first. UI edits append versions only inside the product tenant, preserving the evaluation database and evidence unchanged.
